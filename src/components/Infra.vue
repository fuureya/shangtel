<template>
    <section ref="sectionRef"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                                <div
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <div class="text-2xl font-bold text-gray-900">{{ formatNumber(radio1000) }}+
                                            </div>
                                            <div class="text-sm text-gray-600">{{ currentInfraData.radioLabel }}</div>
                                            <div class="text-xs text-gray-500 mt-1">{{ currentInfraData.radioDesc }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <div class="text-2xl font-bold text-gray-900">{{ formatNumber(kabel20000)
                                                }}+</div>
                                            <div class="text-sm text-gray-600">{{ currentInfraData.kabelLabel }}</div>
                                            <div class="text-xs text-gray-500 mt-1">{{ currentInfraData.kabelDesc }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="absolute top-3/4 right-1/4 w-1 h-1 bg-indigo-400 rounded-full animate-ping delay-1000"></div>
        <div class="absolute bottom-1/4 left-1/3 w-1.5 h-1.5 bg-purple-400 rounded-full animate-ping delay-2000"></div>
    </section>
</template>
...
const animateNumbers = () => {
    ...
    const radio1000Animation = () => {
        if (radio1000.value < radio1000Target && isVisible.value) {
            radio1000.value = Math.min(radio1000.value + radio1000Increment, radio1000Target)
    ...
    const kabel20000Animation = () => {
        if (kabel20000.value < kabel20000Target && isVisible.value) {
            kabel20000.value = Math.min(kabel20000.value + kabel20000Increment, kabel20000Target)
...
const formatNumber = (num) => {
    if (num >= 1000) {
        return Math.floor(num / 1000) + ',' + (Math.floor(num) % 1000).toString().padStart(3, '0')
    }
